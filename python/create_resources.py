...
from fhir.resources.conceptmap import (
    ConceptMap,
    ConceptMapGroup,
...
cs_answers = questionary.form(
    url=questionary.text(
        "Canonical URL of the CodeSystem?", validate=NotEmptyValidator
    ),
    valueSet=questionary.text(
        "Canonical URL of the implicit ValueSet?", validate=NotEmptyValidator
    ),
    ...
    status=questionary.select(
        "Status", choices=["draft", "active", "retired", "unknown"]
    ),
...
code_system.concept = [
    CodeSystemConcept(
        **{
            ...
            "display": c["display"],
            "property": [
                CodeSystemConceptProperty(**{"code": "unit", "valueString": c["unit"]})
            ],
        }
    )
...
vs_answers = questionary.form(
    url=questionary.text(
        "Canonical URL of the LOINC ValueSet?", validate=NotEmptyValidator
    ),
    version=questionary.text("Version?", validate=NotEmptyValidator),
    title=questionary.text("Title (for humans)?", validate=NotEmptyValidator),
    name=questionary.text("Name (for machines), and id?", validate=NotEmptyValidator),
...
cm_answers = questionary.form(
    url=questionary.text(
        "Canonical URL of the ConceptMap?", validate=NotEmptyValidator
    ),
    version=questionary.text("Version?", validate=NotEmptyValidator),
    title=questionary.text("Title (for humans)?", validate=NotEmptyValidator),
    name=questionary.text("Name (for machines), and id?", validate=NotEmptyValidator),
...
for concept in defined_concepts:
    ...
    equivalence = questionary.autocomplete(
        f"Equivalence for {cd} -> {loinc_cd}?",
        choices=equivalence_choices,
        validate=EquivalenceValidator,
    ).ask()
